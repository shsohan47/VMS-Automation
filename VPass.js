...
describe('template spec', () => {
  ...
  function department()
  {
    function validinfo()
    {
     ...
     cy.visit('https://vpass.aamar.digital/');
     //type department name
     cy.get('[name="department_name"]').scrollIntoView().clear();
     cy.get('[name="department_name"]').type("demo name");
     cy.get('[name="department_description"]').scrollIntoView().clear();
     cy.get('[name="department_description"]').type("demo department description");
  
     //submit
     cy.get('.btn.btn-success.btn-sm').scrollIntoView().click();
     cy.get('[name="department_name"]').should("not.be.empty");
     if(cy.get('.container.d-flex.flex-column').should('be.visible'))
     {
      cy.wait(3000);
      if(cy.contains("demo name").scrollIntoView().should("be.visible"))
      {
        cy.contains('tr', 'demon name')
    ...
    {
    //visit department panel
     cy.visit('https://vms-web.aamarpay.dev/department/');
     //type department name
     cy.get('[name="department_name"]').type("");
     cy.get('[name="department_description"]').type("demo department description");
